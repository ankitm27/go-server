...
func InsertUsers(ctx context.Context, indexName string, docType string) {
	...
	client, err := NewElasticClient(context.Background(), false, -1)
	if err != nil {
		fmt.Println("There is some probelm, Please try after some time")
	}
	type User struct {
		UserID    int    `json:"id"`
	...
	for _, userObj := range listUsers {
		_, err := client.Index().Index(indexName).Type(docType).BodyJson(userObj).Do(ctx)
		if err != nil {
...
func GetAll(ctx context.Context, indexName string) {
	client, err := NewElasticClient(context.Background(), false, -1)
	if err != nil {
		fmt.Println("There is some problem, Please try after some time")
	}
...
func GetUserByID(ctx context.Context, userID int, indexName string) User {
	client, err := NewElasticClient(context.Background(), false, -1)
	if err != nil {
		fmt.Println("There is some problem, Please try after some time")
	}
...
func GetAllActiveUsers(ctx context.Context, indexName string) []User {
	client, err := NewElasticClient(context.Background(), false, -1)
	if err != nil {
		fmt.Println("There is some problem, Please try after some time")
	}
